#!/usr/bin/env python3
"""
ACS 5-Year Data Profile - FULL DP04 Housing Characteristics Downloader

Downloads ALL DP04 (Housing Characteristics) variables from the U.S. Census API
...
Usage:
    ...
    python download_dp04_full.py --state 01 --year 2020   # Alabama, 2020
    python download_dp04_full.py --year 2020              # All states, 2020
    python download_dp04_full.py                          # All states, all years (2009-2020)
"""
...
OUTPUT_DIR = "./data/acs_downloads/housing"
...
def get_all_dp04_variables(year: int) -> dict:
    """
    Fetch ALL DP04 variable codes and their labels from the Census API.
    Returns dict mapping variable code to human-readable label.
    """
    print(f"Fetching DP04 variable definitions for {year}...")
    ...
    try:
        ...
        for var_code, var_info in data["variables"].items():
            # Only get DP04 estimate variables (ending in E, not PE or ME)
            if (var_code.startswith("DP04_") and 
        ...
        print(f"  Found {len(variables)} DP04 estimate variables")
        return variables
...
def download_dp04_batch(state_fips: str, var_codes: List[str], api_key: Optional[str], year: int, max_retries: int = 3) -> Optional[pd.DataFrame]:
    """
    Download a batch of DP04 variables for all counties in a state.
...
def download_dp04_full(state_fips: str, state_name: str, variables: dict, api_key: Optional[str], year: int) -> Optional[pd.DataFrame]:
    """
    Download ALL DP04 variables for all counties in a state.
    Makes multiple API calls in batches to avoid URL length limits.
    """
    print(f"\nDownloading FULL DP04 for {state_name} (FIPS: {state_fips}), Year: {year}...")
    print(f"  Total variables: {len(variables)}")
    
    var_codes = sorted(variables.keys())
    
    # Batch size - reduced to 35 for more reliable requests (DP04 has many variables)
    BATCH_SIZE = 35
    ...
    for i in range(0, len(var_codes), BATCH_SIZE):
        ...
        df = download_dp04_batch(state_fips, batch, api_key, year)
...
def main():
    parser = argparse.ArgumentParser(
        description="Download FULL ACS 5-Year DP04 Housing Characteristics data"
    )
    ...
    print("=" * 70)
    print("ACS 5-Year DP04 FULL Housing Characteristics Downloader")
    print("=" * 70)
    ...
    for year in years_to_download:
        ...
        variables = get_all_dp04_variables(year)
        if not variables:
        ...
        for state_fips, state_name in states_to_download.items():
            df = download_dp04_full(state_fips, state_name, variables, api_key, year)
            
            if df is not None:
                # Add year column
                df.insert(0, "year", year)
                
                filename = f"{state_fips}_{state_name}_DP04_Housing_FULL.csv"
