#!/usr/bin/env python3
"""
ACS 5-Year Data Profile - FULL DP02 Social Characteristics Downloader

Downloads ALL DP02 (Social Characteristics) variables from the U.S. Census API
...
Usage:
    ...
    python download_dp02_full.py --state 01 --year 2020   # Alabama, 2020
    python download_dp02_full.py --year 2020              # All states, 2020
    python download_dp02_full.py                          # All states, all years (2009-2020)
"""
...
OUTPUT_DIR = "./data/acs_downloads/social"
...
def get_all_dp02_variables(year: int) -> dict:
    """
    Fetch ALL DP02 variable codes and their labels from the Census API.
    Returns dict mapping variable code to human-readable label.
    """
    print(f"Fetching DP02 variable definitions for {year}...")
    ...
    try:
        ...
        for var_code, var_info in data["variables"].items():
            # Only get DP02 estimate variables (ending in E, not PE or ME)
            if (var_code.startswith("DP02_") and 
        ...
        print(f"  Found {len(variables)} DP02 estimate variables")
        return variables
...
def download_dp02_batch(state_fips: str, var_codes: List[str], api_key: Optional[str], year: int) -> Optional[pd.DataFrame]:
    """
    Download a batch of DP02 variables for all counties in a state.
...
def download_dp02_full(state_fips: str, state_name: str, variables: dict, api_key: Optional[str], year: int) -> Optional[pd.DataFrame]:
    """
    Download ALL DP02 variables for all counties in a state.
    Makes multiple API calls in batches to avoid URL length limits.
    """
    print(f"\nDownloading FULL DP02 for {state_name} (FIPS: {state_fips}), Year: {year}...")
    ...
    for i in range(0, len(var_codes), BATCH_SIZE):
        ...
        df = download_dp02_batch(state_fips, batch, api_key, year)
...
def main():
    parser = argparse.ArgumentParser(
        description="Download FULL ACS 5-Year DP02 Social Characteristics data"
    )
    ...
    print("=" * 70)
    print("ACS 5-Year DP02 FULL Social Characteristics Downloader")
    print("=" * 70)
    ...
    for year in years_to_download:
        ...
        variables = get_all_dp02_variables(year)
        if not variables:
        ...
        for state_fips, state_name in states_to_download.items():
            df = download_dp02_full(state_fips, state_name, variables, api_key, year)
            
            if df is not None:
                # Add year column
                df.insert(0, "year", year)
                
                filename = f"{state_fips}_{state_name}_DP02_Social_FULL.csv"
